...
function draw() {
    ...
    if(startGame === false){
      ...
      ctx.fillText("Press spacebar to start or restart the game" , canvas.width / 2 , canvas.height /2 + 100);
      ctx.fillText("Press x key to stop the game" , canvas.width / 2 , canvas.height /2 + 130);
   }
}
...
function update() {
    ...
    // check if ball hits top or bottom wall. If this happens, the ball is reset
...
window.addEventListener("keydown", function(ev){
  if(ev.keyCode === 32) startGame = true;
  else if(ev.keyCode === 88 && startGame === true) startGame = false;
});
